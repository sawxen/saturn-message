...
import { FiClock, FiUserPlus, FiCheck } from 'react-icons/fi';
...
const ChatList: React.FC<ChatListProps> = ({
    ...
    const [, setErrorMessages] = React.useState<Record<string, string>>({});

    // Функция для загрузки последнего сообщения
    const loadLastMessage = React.useCallback(
        async (chatId: string) => {
            ...
            try {
                ...
                setErrorMessages((prev) => ({
                    ...prev,
                    [chatId]: 'Не удалось загрузить сообщение',
                }));
    ...
    const getChatLastMessage = (chat: Chat) => {
        ...
        // Если идет загрузка или ошибка, не показываем чат
        return null;
    };

    // Фильтрация чатов: показываем только те, у которых есть последнее сообщение
                ...
                (chat) =>
                    (chat.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
                        (chat.lastMessage?.text
                            ...
                            false)) &&
                    getChatLastMessage(chat) // Только чаты с последним сообщением
            )
            : chats.filter((chat) => getChatLastMessage(chat)) // Только чаты с последним сообщением
    );
